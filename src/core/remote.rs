...
impl RemoteResources {
    ...
    pub async fn report_pullrequests(&self, user_calls: &str, opened_assignee: bool, unreviewed: bool) -> rq::Result<Cow<'static, str>> {
    ...
    pub async fn report_unresolved_issues(&self) -> rq::Result<Cow<'static, str>> {
...
#[derive(serde::Deserialize, Debug)] pub struct RepositoryInfo {
    pub id: usize
}
#[derive(serde::Deserialize, Debug)] pub struct PullRequestUrls { pub html_url: String }
#[derive(Debug, serde::Deserialize)] pub struct IssueInfo {
    pub html_url: String, pub number: usize, pub title: String, pub body: String,
    pub pull_request: Option<PullRequestUrls>
}
#[derive(Debug, serde::Deserialize)] pub struct PullRequestInfo {
...
#[derive(Debug, serde::Deserialize)] pub struct UserInfo {
    pub login: String, pub id: u64
}
#[derive(Debug, serde::Deserialize)] pub struct FullUserInfo {
    pub login: String, pub id: u64, pub name: String
}
#[derive(Debug, serde::Deserialize)] pub struct ExtraIssueInfo {
    pub is_epic: bool, pub pipeline: PipelineInfo
}
#[derive(Debug, serde::Deserialize)] pub struct PipelineInfo { pub name: String }

#[derive(Debug, serde::Deserialize)] #[serde(untagged)] pub enum GitHubResponse<T> {
...
impl<'g> RemoteRepository<'g> {
    pub async fn opened_issues_by_assignee(&self, assignee: &str) -> rq::Result<GitHubResponse<Vec<IssueInfo>>> {
    ...
    pub async fn opened_issues_by_labeled(&self, labels: &str) -> rq::Result<GitHubResponse<Vec<IssueInfo>>> {
        self.0.github_auth_get(&format!("https://api.github.com/repos/{}/issues", self.1))
            .query(&[("labels", labels), ("state", "open"), ("sort", "updated"), ("direction", "asc")])
            .send().await?.json().await
    }
    pub async fn issue_on_board(&self, num: usize) -> rq::Result<GitHubResponse<ExtraIssueInfo>> {
        self.0.zenhub_auth_get(&format!("https://api.zenhub.io/p1/repositories/{}/issues/{}", self.2, num))
            .send().await?.json().await
    }
    pub async fn pullrequests(&self) -> rq::Result<GitHubResponse<Vec<PullRequestInfo>>> {
        self.0.github_auth_get(&format!("https://api.github.com/repos/{}/pulls", self.1))
            .send().await?.json().await
    }
    pub async fn unresolved_issues(&self) -> rq::Result<GitHubResponse<Vec<IssueInfo>>> {
...
impl RemoteProjectHub {
    pub async fn user(&self, user_login_name: &str) -> rq::Result<GitHubResponse<RemoteUser>> {
        self.github_auth_get(&format!("https://api.github.com/users/{}", user_login_name))
            .send().await?
            .json().await
            .map(|j: GitHubResponse<_>| j.map(|jm| RemoteUser(self.clone(), jm)))
    }
}
impl RemoteUser {
