...
impl RemoteResources {
    ...
    pub async fn collect_remaining_tasks<'s>(
        &self,
        user: super::User<'s>,
    ) -> super::ProcessResult<Vec<CurrentTaskState>> {
        let (_, ghlogin) = user.identify_as_github_id(&self.persistent)?;
        let works = self
            .repo
            .opened_issues_by_assignee(&ghlogin)
            .await?
            .into_iter()
            .map(|s| async {
                let exinfo = self.repo.issue_on_board(s.number).await?;
                ...
                Ok(CurrentTaskState {
                    ...
                    title: s.title,
                    url: s.html_url,
                    progress: (checked_count, total_checks),
                    is_pr: s.pull_request.is_some(),
                    pipeline_stage_name: exinfo.pipeline.name,
                })
            });
        futures_util::future::join_all(works)
            .await
            .into_iter()
            .collect::<Result<Vec<_>, _>>()
    }

    pub async fn greeting<'s>(
        &self,
        user: super::User<'s>,
        following_desumasu: bool,
    ) -> super::ProcessResult<String> {
        ...
        if self.persistent.is_user_working(ghid) {
            ...
            let tasklist = self
                .collect_remaining_tasks(super::User::GitHub(ghid, ghlogin))
                .await?;
            self.persistent.user_setup_work(
                ghid,
                super::utcnow(),
                &tasklist
                    .iter()
                    .map(|x| {
                        let (a, b) = x.progress;
                        super::RemainingWork {
                            issue_id: x.number as _,
                            progress: Some((a as _, b as _)),
                        }
                    })
                    .collect::<Vec<_>>(),
            );
    ...
    pub fn greeting_message(
        &self,
        tasklist: Option<Vec<CurrentTaskState>>,
        following_desumasu: bool,
    ) -> super::ProcessResult<String> {
        ...
        if let Some(tasklist) = tasklist {
            ...
            if tasklist.is_empty() {
                ...
                hello_text2 = *if following_desumasu {
                    ...
                    [
                        "今残ってるタスクはこれだよ。",
                        "今日の献立だよー:issue-o::pr::issue-o::pr:",
                    ]
                    .choose(&mut rand::thread_rng())
                    .unwrap()
                };
                tasklines = tasklist
                    .into_iter()
                    .map(|x| TaskAchievementWriter(x).to_string())
                    .collect();
                after_text = *if following_desumasu {
                    [
                        "それじゃ、今日も張り切って行きましょー！！",
    ...
    pub async fn count_source<'s, 'u>(
        &self,
        what: &'s super::TellWhat,
        user: super::User<'u>,
    ) -> super::ProcessResult<(usize, &'s str, bool)> {
        match what {
            super::TellWhat::Issues {
                unresolved: true,
                user_calls,
                ask_for_remains,
            } => Ok((
                self.repo
                    .opened_issues_by_labeled("UnresolvedQuestions")
                    .await?
                    .len(),
                user_calls as _,
                *ask_for_remains,
            )),
            super::TellWhat::Tasks(user_calls, afr, _) => Ok((
                self.collect_remaining_tasks(user).await?.len(),
                user_calls as _,
                *afr,
            )),
    ...
    pub fn report_count(&self, count: usize, user_calls: &str, ask_for_remains: bool) -> String {
        format!(
            "{}は{}{}個だよー",
            user_calls,
            if ask_for_remains { "あと" } else { "" },
            count
        )
    }
    pub async fn report_tasks<'s>(
        &self,
        user_calls: &str,
        user: super::User<'s>,
    ) -> super::ProcessResult<String> {
        let tasklines: Vec<_> = self
            .collect_remaining_tasks(user)
            .await?
            .into_iter()
            .map(TaskAchievementWriter)
            .map(|x| x.to_string())
            .collect();
    ...
    pub async fn report_pullrequests(
        &self,
        user_calls: &str,
        opened_assignee: bool,
        unreviewed: bool,
    ) -> surf::Result<Cow<'static, str>> {
        let prs = self.repo.pullrequests().await?;
        if !opened_assignee && !unreviewed {
            ...
            Ok(if let Some(pr) = pr {
                format!(
                    "はい。\n{}",
                    OpenedPullRequestFormatter::new(pr, &self.repo).await
                )
                .into()
            } else {
                Cow::from(
                ...
                (opened_assignee && p.assignees.iter().find(|a| a.login == "Pctg-x8").is_some())
                    || (unreviewed
                        && p.requested_reviewers
                            .iter()
                            .find(|a| a.login == "Pctg-x8")
                            .is_some())
            });
            let filtered = futures_util::future::join_all(
                filtered_elements.map(|p| OpenedPullRequestFormatter::new(p, &self.repo)),
            )
            .await;
    ...
    pub async fn report_unresolved_issues(&self) -> surf::Result<Cow<'static, str>> {
        let issues = self.repo.unresolved_issues().await?;
        if issues.is_empty() {
            Ok("未解決の課題はないかなぁ".into())
        } else {
...
impl Display for TaskAchievementWriter {
    fn fmt(&self, fmt: &mut Formatter) -> FmtResult {
        let stage_str = if self.0.pipeline_stage_name == "In Progress" {
            format!("*{} ステージ*", self.0.pipeline_stage_name)
        } else {
            format!("{} ステージ", self.0.pipeline_stage_name)
        };
        let miscinfo = if self.0.progress.1 == 0 {
            stage_str
        } else {
            format!(
                "{}, {}個中{}個完了",
                stage_str, self.0.progress.1, self.0.progress.0
            )
        };
        ...
        write!(
            fmt,
            "<{}|:{}:#{}>: {} ({})",
            self.0.url, icon, self.0.number, self.0.title, miscinfo
        )
    }
}
    ...
    fn fmt(&self, fmt: &mut Formatter) -> FmtResult {
        ...
        let miscinfo = if self.0.progress.1 == 0 {
            ...
            format!(
                ...
                stage_str,
                self.0.progress.1,
                self.0.progress.1 - self.0.progress.0
            )
        };
        let icon = if self.0.is_pr { "pr" } else { "issue-o" };
...
impl<'p> Display for OpenedPullRequestFormatter<'p> {
    fn fmt(&self, fmt: &mut Formatter) -> FmtResult {
        ...
        let miscinfo = if self.3 == 0 {
            stage_str
        } else {
            format!("{}, {}個中{}個完了", stage_str, self.2, self.3)
        };

        write!(
            fmt,
            "<{}|#{}>: {} ({})",
            self.0.html_url, self.0.number, self.0.title, miscinfo
        )
    }
}
impl<'p> OpenedPullRequestFormatter<'p> {
    async fn new<'r>(
        pr: &'p PullRequestInfo,
        repo: &RemoteRepository<'r>,
    ) -> OpenedPullRequestFormatter<'p> {
        let einfo = repo.issue_on_board(pr.number).await.unwrap();
...
fn github_api_token() -> String {
    std::env::var("GITHUB_API_TOKEN").expect("GITHUB_API_TOKEN not set")
}
fn zenhub_api_token() -> String {
    std::env::var("ZENHUB_API_TOKEN").expect("ZENHUB_API_TOKEN not set")
}

pub struct RemoteProjectHubGate {
    c: surf::Client,
    auth: String,
}
#[derive(Clone)]
...
impl RemoteProjectHub {
    pub fn new() -> Self {
        RemoteProjectHub(Arc::new(RemoteProjectHubGate {
            c: surf::Client::new(),
            auth: format!("token {}", github_api_token()),
        }))
    }
    ...
    fn github_auth_get(&self, url: impl AsRef<str>) -> surf::RequestBuilder {
        self.0
            .c
            .get(url)
            .header(surf::http::headers::AUTHORIZATION, &self.0.auth[..])
    }
    ...
    fn zenhub_auth_get(&self, url: impl AsRef<str>) -> surf::RequestBuilder {
        self.0
            .c
            .get(url)
            .header("X-Authentication-Token", zenhub_api_token())
    }
}
...
impl<T> GitHubResponse<T> {
    pub fn map<F, U>(self, mapper: F) -> GitHubResponse<U>
    where
        F: FnOnce(T) -> U,
    {
        match self {
    ...
    pub fn unwrap(self) -> T {
        match self {
            ...
            GitHubResponse::Failed { message } => {
                panic!("Unwrapping GitHubResponse Failed! {}", message)
            }
        }
    }
}
...
impl RemoteProjectHub {
    ...
    pub async fn repository<'g>(&self, repo: &'g str) -> RemoteRepository<'g> {
        let repoinfo: RepositoryInfo = self
            .github_auth_get(&format!("https://api.github.com/repos/{}", repo))
            .send()
            .await
            .unwrap()
            .body_json()
...
impl<'g> RemoteRepository<'g> {
    pub async fn opened_issues_by_assignee(&self, assignee: &str) -> surf::Result<Vec<IssueInfo>> {
        let resp = self
            .0
            .github_auth_get(format!("https://api.github.com/repos/{}/issues", self.1))
            .query(&[
                ("assignee", assignee),
                ("state", "open"),
                ("sort", "updated"),
            ...
            .recv_string()
            .await?;
        serde_json::from_str(dbg!(&resp)).map_err(From::from)
    }
    pub async fn opened_issues_by_labeled(&self, labels: &str) -> surf::Result<Vec<IssueInfo>> {
        let resp = self
            .0
            .github_auth_get(format!("https://api.github.com/repos/{}/issues", self.1))
            .query(&[
    ...
    pub async fn issue_on_board(&self, num: usize) -> surf::Result<ExtraIssueInfo> {
        let resp = self
            .0
            .zenhub_auth_get(format!(
                "https://api.zenhub.io/p1/repositories/{}/issues/{}",
                self.2, num
            ))
            .recv_string()
            .await?;
    ...
    pub async fn pullrequests(&self) -> surf::Result<Vec<PullRequestInfo>> {
        self.0
            .github_auth_get(format!("https://api.github.com/repos/{}/pulls", self.1))
            .send()
            .await?
            .body_json()
            .await
    }
    pub async fn unresolved_issues(&self) -> surf::Result<Vec<IssueInfo>> {
...
impl RemoteProjectHub {
    pub async fn user(&self, user_login_name: &str) -> surf::Result<GitHubResponse<RemoteUser>> {
        let resp: GitHubResponse<_> = self
            .github_auth_get(format!("https://api.github.com/users/{}", user_login_name))
            .send()
            .await?
            .body_json()
            .await?;
...
impl RemoteUser {
    pub fn response(&self) -> &FullUserInfo {
        &self.1
    }
}
